...
const Conversation = ({item}: {item: Props}) => {
  return (
    <Flex align="center" className='m-3 mb-0 w-full h-16 rounded ml-0 px-3 cursor-pointer border-b border-b-gray-800 hover:bg-teal-950'>
      ...
      <Flex direction="column" justify="end" className='h-full pb-3'>
        <p className='text-xs text-gray-300'>{item.lastMsg.createdAt ? formatTime(item.lastMsg.createdAt) : ''}</p>
      </Flex>
    </Flex>
  )
}
